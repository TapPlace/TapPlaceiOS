...
extension TermsViewController: CustomNavigationBarProtocol, BottomButtonProtocol {
    func didTapBottomButton() {
        ...
        if let _ = storageViewModel.getUserInfo(uuid: Constants.keyChainDeviceID) {
...
//MARK: - TableView
extension TermsViewController: UITableViewDelegate, UITableViewDataSource, TermsProtocol {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.row == allTermsLists.count - 1 {
            return 60
        } else {
            return 40
        }
